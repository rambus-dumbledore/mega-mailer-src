...
type PGPool =
    bb8::Pool<bb8_postgres::PostgresConnectionManager<bb8_postgres::tokio_postgres::NoTls>>;
...
impl Storage {
    pub async fn new(cfg: &StorageCfg) -> Result<Self> {
        ...
        let pg_man = bb8_postgres::PostgresConnectionManager::new(
            pg_config,
    ...
    pub async fn set_mail_account(
        ...
        conn.execute(
            &statement,
            &[
                &encrypted_account.email,
                &encrypted_account.password,
                &user.id,
            ],
        )
        .await?;
    ...
    pub async fn set_user_working_hours(&self, user: &WebAppUser, wh: &[u8; 2]) -> Result<()> {
       ...
       "#,
            ...
            .await?;
        conn.execute(&statement, &[&(wh[0] as i32), &(wh[1] as i32), &user.id])
            .await?;
        Ok(())
    }
    ...
    pub async fn set_important_emails(
        &self,
        user: &WebAppUser,
        emails: &Vec<String>,
    ) -> Result<bool> {
    ...
    pub async fn set_heartbeat(&self, service: &String, timestamp: i64) -> Result<bool> {
        let mut conn = self.redis.get().await?;
        let res = conn
            .hset(&String::from("HEARTBEAT"), service, timestamp)
            .await?;
        Ok(res)
    }
